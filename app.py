...
from rummy import RummyMatch, rummy_matches
...
# --- RUMMY SOCKET.IO HANDLERS ---
rummy_timers = {}

def cancel_rummy_timer(match_id):
    if match_id in rummy_timers:
        timer = rummy_timers.pop(match_id)
        try:
            timer.cancel()
        except Exception:
            pass

def start_rummy_turn_timer(match_id):
    cancel_rummy_timer(match_id)
    # 35-second timer
    timer = threading.Timer(35.0, execute_rummy_timeout, args=[match_id])
...
def execute_rummy_timeout(match_id):
    with app.app_context():
        ...
        active_player = match.players[match.current_turn_index]
        user_id = active_player["user_id"]
        
        if match.turn_state == "draw":
            success, card_or_err = match.draw_card(user_id, "deck")
            if success:
...
def handle_create_rummy_room(data):
    ...
    while attempts < 100:
        ...
        code_exists = any(m.room_code == code and m.status == "waiting" for m in rummy_matches.values())
        if not code_exists:
...
def handle_join_rummy_room(data):
    user_id = int(data["userId"])
    username = data.get("username", f"User_{user_id}")
    room_code = str(data["roomCode"]).strip()
    
    match = None
    for m in rummy_matches.values():
        if m.room_code == room_code and m.status == "waiting":
            match = m
            break
            
    if not match:
        emit("rummy_error", {"message": "Active room with this code was not found."})
        return
        
    if len(match.players) >= match.max_players:
        emit("rummy_error", {"message": "Room is already full."})
        return
...
@socketio.on("start_rummy_game")
def handle_start_rummy_game(data):
    match_id = data["matchId"]
    user_id = int(data["userId"])
    
    match = rummy_matches.get(match_id)
    if not match:
        emit("rummy_error", {"message": "Room not found."})
        return
        
    if match.creator_id != user_id:
        emit("rummy_error", {"message": "Only the host can start the game."})
        return
...
@socketio.on("rummy_draw_card")
def handle_rummy_draw_card(data):
    match_id = data["matchId"]
    user_id = int(data["userId"])
    source = data["source"]
    ...
    match = rummy_matches.get(match_id)
    if not match or match.status != "playing":
        emit("rummy_error", {"message": "Game is not currently active."})
        return
...
@socketio.on("rummy_discard_card")
def handle_rummy_discard_card(data):
    match_id = data["matchId"]
    user_id = int(data["userId"])
    card = data["card"]
    ...
    match = rummy_matches.get(match_id)
    if not match or match.status != "playing":
        emit("rummy_error", {"message": "Game is not active."})
        return
...
@socketio.on("rummy_sync_groups")
def handle_rummy_sync_groups(data):
    match_id = data["matchId"]
    user_id = int(data["userId"])
    groups = data["groups"]
    ...
    match = rummy_matches.get(match_id)
    if match:
        match.update_groups(user_id, groups)

@socketio.on("rummy_declare")
def handle_rummy_declare(data):
    match_id = data["matchId"]
    user_id = int(data["userId"])
    groups = data["groups"]
...
@socketio.on("leave_rummy_room")
def handle_leave_rummy_room(data):
    match_id = data["matchId"]
    user_id = int(data["userId"])
    
    match = rummy_matches.get(match_id)
    if match:
        is_empty = match.remove_player(user_id)
        if is_empty:
            cancel_rummy_timer(match_id)
